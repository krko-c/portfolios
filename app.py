...
def render_help():
    ...
    with tab3:
        ...
        with th:
            ...
            st.markdown(
                "차감되는 항을 **변동성 손실(volatility drag)** 이라 합니다.\n\n"
                "+50% 뒤 −50%면 산술평균은 0%지만 실제로는 원금의 75%만 남습니다. "
                "변동성이 크면 같은 평균 수익률이어도 손에 쥐는 돈이 줄어듭니다. "
            ...
            st.markdown(
                "| 하락 | 필요한 상승 |\n|---|---|\n| −10% | +11% |\n| −20% | +25% |\n"
                "| −30% | +43% |\n| −50% | +100% |\n| −70% | +233% |\n\n"
                "최대낙폭(MDD)이 단순 참고치가 아니라 **복구 가능성의 문제**인 이유입니다. "
            ...
            st.markdown(
                ...
                "**'공짜 점심'이라 불리는 이유가 여기 있습니다.** 기대수익은 비중의 "
            ...
            st.divider()
            st.subheader("5. 최적화의 함정 — 왜 한 종목에 몰빵이 나오는가")
            st.markdown(
                "평균-분산 최적화를 그대로 돌리면 **한두 종목에 극단적으로 쏠리는** 결과가 "
                "자주 나옵니다. 위험을 무시해서가 아니라 **기대수익률 추정이 부정확하기 "
                "때문**입니다. 평균의 추정 오차는 다음과 같습니다.")
            ...
            st.markdown(
                ...
                "그 결과 최적화기는 *진짜 좋은 자산*이 아니라 *추정 오차가 위쪽으로 크게 튄 "
                "자산*을 골라냅니다. Michaud는 이를 두고 평균-분산 최적화가 "
                "**'오차 극대화기(error maximizer)'** 라고 표현했습니다.")
            st.warning("**대응 방법 세 가지**\n\n"
                       "1. **최대 편입비중 제약** — 한 종목이 40%를 넘지 못하게 묶습니다\n"
                       "2. **학습 기간 확대** — 추정 오차를 줄입니다\n"
                       "3. **기대수익률을 쓰지 않는 방식** — 변동성 최소화 · 위험균형 · HRP는 "
                       "평균 추정을 아예 하지 않아 훨씬 안정적입니다\n\n"
                       "**비중 안정성 점검**에서 학습 기간을 바꿨을 때 비중이 크게 흔들린다면, "
                       "그 결과는 추정 오차에 기댄 것일 가능성이 높습니다.")
            ...
            st.markdown(
                "변동성이 큰 자산은 자동으로 비중이 낮아집니다. 흔한 60/40 포트폴리오는 "
                "비중은 6:4지만 위험 기여도로는 주식이 90% 이상을 차지하는데, 위험균형은 "
                "이 불균형을 바로잡습니다.\n\n"
            ...
            st.divider()
            st.subheader("7. 검증 — 표본내와 표본외")
            st.markdown(
                "과거 전체를 보고 최적 비중을 고른 뒤 그 기간 성과를 자랑하는 것은 "
                "답안지를 보고 시험을 치는 것과 같습니다. 이를 **과최적화**라 합니다. "
                "그래서 기간을 둘로 나눕니다.")
            st.code("├────── 학습 기간 ──────┤────── 검증 기간 ──────┤\n"
        ...
        with pr:
            st.subheader("리밸런싱")
            st.markdown(
                "가격이 움직이면 비중은 저절로 흘러갑니다. 오른 자산의 비중이 커지죠. "
                "리밸런싱은 이를 목표 비중으로 되돌리는 것입니다.\n\n"
                "| 방식 | 조정 시점 |\n|---|---|\n"
                "| 매일 (고정비중 유지) | 매 거래일. 비중이 전혀 흐르지 않음 |\n"
                "| 월별 / 분기별 / 연별 | 각 구간 첫 거래일에만 되돌림 |\n"
                "| 없음 | 첫날 사고 그대로 보유 |\n\n"
                "**결과가 크게 달라집니다.** 매수 후 보유는 오른 자산에 비중이 쏠려 "
                "최대낙폭이 훨씬 커지는 경향이 있습니다. 반대로 자주 리밸런싱하면 회전율이 "
                "올라 비용 부담이 커집니다. 실무에서는 분기별이 흔한 절충안입니다.\n\n"
                "리밸런싱은 본질적으로 **오른 것을 팔아 내린 것을 사는** 행위이므로, "
                "평균회귀 성향의 시장에서는 수익에도 도움이 되지만 추세가 길게 이어지는 "
                "시장에서는 불리할 수 있습니다.")
            ...
            st.markdown(
                ...
                "잦은 자산의 표준편차는 거의 같게 나오는 반면, CDaR로 재면 몇 배씩 차이가 "
                "납니다. 낙폭을 견디기 어려운 성향이라면 낙폭 기반 지표가 목적에 맞습니다.")
