...
def render_help():
    ...
    with tab1:
        ...
        st.markdown(
            "여러 종목을 조합한 **포트폴리오가 과거에 어떤 성과를 냈는지** 계산하고, "
            "**어떤 비중이 좋았을지** 찾아주는 도구입니다. "
            "화면은 두 개입니다.\n\n"
            "- **📊 포트폴리오 분석** — 내가 정한 비중으로 과거 성과를 확인합니다. "
            "여러 조합과 벤치마크를 한 화면에서 비교할 수 있습니다.\n"
            "- **🎯 포트폴리오 최적화** — 목적(예: 위험 대비 수익 최대화)을 정하면 "
            "그에 맞는 비중을 계산하고, 지금 내 비중과 비교해줍니다.")
        ...
        st.markdown(
            ...
            "표본내(학습 구간) 성과는 좋게 나오는 것이 당연하므로 판단 근거로 삼지 마세요.")
    ...
    with tab3:
        st.subheader("리밸런싱")
        st.markdown(
            "비중은 가격이 움직이면 저절로 흘러갑니다. 오른 자산의 비중이 커지죠. "
            "리밸런싱은 이를 목표 비중으로 되돌리는 것입니다.\n\n"
            "| 방식 | 조정 시점 |\n|---|---|\n"
            "| 매일 (고정비중 유지) | 매 거래일. 비중이 전혀 흐르지 않음 |\n"
            "| 월별 / 분기별 / 연별 | 각 구간 첫 거래일에만 되돌림 |\n"
            "| 없음 | 첫날 사고 그대로 보유 |\n\n"
            "**왜 중요한가** — 같은 종목·비중이어도 결과가 크게 달라집니다. "
            "매수 후 보유는 오른 자산에 비중이 쏠려 **최대낙폭이 훨씬 커지는** 경향이 "
            "있습니다. 반대로 자주 리밸런싱하면 회전율이 높아져 거래비용이 늘어납니다.")

        st.divider()
        st.subheader("배당 처리")
        st.markdown(
            "**배당 재투자**는 받은 배당으로 같은 종목을 다시 산다고 가정합니다(총수익). "
            "**주가만**은 배당을 제외하고 가격 변동만 봅니다.\n\n"
            "배당수익률 2.5% 종목을 10년 보유하면 두 방식의 최종 자산이 약 28% 차이 납니다. "
            "장기 분석에서는 재투자 기준이 표준입니다. 두 경우 모두 액면분할은 보정되므로 "
            "분할일에 가짜 폭락이 생기지 않습니다.")

        st.divider()
        st.subheader("통화 환산과 환헤지")
        st.markdown(
            "국가가 다른 종목을 섞으면 **기준 통화로 환산**해야 비교가 됩니다. "
            "이때 환율 변동도 수익률에 포함됩니다. 원화 기준으로 미국 주식을 보면 "
            "주가가 그대로여도 환율이 오르면 수익이 납니다.\n\n"
            "**환헤지 가정**을 켜면 환율 변동을 제거하고 종목 자체 수익률만 봅니다. "
            "실제로 환헤지를 하려면 비용이 들지만, 이 도구는 비용 없이 완전히 헤지된 "
            "상황을 가정합니다.")
        st.warning("엔화 환율은 **1엔당 원화**입니다. 뉴스의 '100엔당' 표기와 100배 차이나므로 "
                   "숫자가 9~10 정도로 보여도 정상입니다.")

        st.divider()
        st.subheader("표본내와 표본외 — 최적화에서 가장 중요한 개념")
        st.markdown(
            "과거 전체를 보고 '무엇이 좋았는지' 고르면 당연히 좋아 보입니다. "
            "답을 알고 시험을 치는 것과 같기 때문입니다.\n\n"
            "그래서 이 도구는 기간을 둘로 나눕니다.\n\n"
            "- **학습 기간(표본내)** — 이 구간 데이터로만 비중을 계산합니다\n"
            "- **최적화 기준일 이후(표본외)** — 계산된 비중을 그대로 적용해 실제 성과를 잽니다\n\n"
            "**표본외 성과가 진짜 성적표입니다.** 최적화가 원본보다 나쁘게 나오는 경우도 "
            "흔한데, 그것이 정직한 결과입니다. 표본내에서만 좋고 표본외에서 나쁘다면 "
            "**과최적화**된 것입니다.")
        ...
        st.markdown(
            ...
            "| **위험 대비 수익 최대화** | 선택한 위험 지표로 나눈 초과수익을 극대화합니다. "
            "위험 지표를 표준편차로 두면 샤프지수 최대화와 같습니다 |\n"
            "| **위험 최소화** | 수익과 무관하게 위험만 줄입니다 |\n"
            "| **수익률 최대화** | 제약 안에서 기대수익만 극대화합니다 |\n"
            "| **위험균형 (Risk Parity)** | 각 종목의 위험 기여도를 같게 만듭니다. "
            "변동성이 큰 종목의 비중이 자연히 낮아집니다 |\n"
            "| **계층적 위험 분산 (HRP)** | 상관관계로 종목을 묶고 트리를 따라 역분산 배분합니다 |\n")
        st.info("**HRP가 특별한 이유** — 다른 방식은 '이 종목이 앞으로 연 몇 %를 낼 것인가'를 "
                "과거 평균으로 추정해 씁니다. 그런데 이 추정은 오차가 매우 커서 "
                "과최적화의 주범입니다. HRP는 기대수익률을 아예 쓰지 않고 "
                "상관구조와 변동성만으로 배분하므로, 표본외에서 상대적으로 안정적입니다.")

        st.divider()
        st.subheader("위험을 정의하는 여러 방법")
        st.markdown(
            "'위험'은 하나가 아닙니다. 무엇을 위험으로 볼지에 따라 최적 비중이 달라집니다.\n\n"
            "| 그룹 | 지표 | 관점 |\n|---|---|---|\n"
            "| 변동성 기반 | 표준편차, 평균절대편차 | 얼마나 흔들리는가 |\n"
            "| 하방위험 | 준표준편차, 소르티노, 오메가 | 내려갈 때만 위험이다 |\n"
            "| 낙폭 기반 | 조건부 낙폭(CDaR), 얼서지수 | 얼마나 깊이·오래 물리는가 |\n"
            "| 꼬리위험 | 조건부 손실(CVaR) | 최악일 때 얼마나 잃는가 |\n\n"
            "**왜 여러 개가 필요한가** — 표준편차는 급등과 급락을 구분하지 못합니다. "
            "실제로 상승이 잦은 자산과 하락이 잦은 자산의 표준편차가 거의 같게 나오는 반면, "
            "CDaR로 재면 몇 배씩 차이가 납니다. 낙폭을 견디기 어렵다면 낙폭 기반 지표를 "
            "고르는 것이 목적에 맞습니다.")

        st.divider()
        st.subheader("주기적 재최적화 (워크포워드)")
        st.markdown(
            "한 번 정한 비중을 끝까지 유지하는 대신, **정해진 주기마다 그 시점까지의 "
            "데이터로 비중을 다시 계산**해 갈아끼웁니다.\n\n"
            "각 시점에서 미래 정보를 전혀 쓰지 않으므로, **실제로 그렇게 운용했다면 "
            "어땠을지**를 재현합니다. 시장 주도주가 바뀌는 국면에서는 고정 비중보다 "
            "크게 유리할 수 있지만, 거래가 잦아져 비용이 늘어납니다. "
            "사이드바에 거래비용을 입력해두면 그 영향까지 반영됩니다.")

        st.divider()
        st.subheader("거래비용")
        st.markdown(
            "**편도 bp**로 입력합니다. 1bp = 0.01%이며, 매수와 매도 각각에 적용됩니다. "
            "국내주식은 위탁수수료와 증권거래세를 합쳐 보통 20~30bp 수준입니다.\n\n"
            "비용은 **회전율에 비례**합니다. 매수 후 보유는 회전율이 0이라 영향이 없지만, "
            "매일 리밸런싱은 연 200%를 넘겨 성과가 눈에 띄게 깎입니다.")

        st.divider()
        st.subheader("이 도구의 한계")
        st.markdown(
            "- **과거가 미래를 보장하지 않습니다.** 특히 최적화는 과거의 평균·변동성·"
            "상관관계가 앞으로도 유지된다고 가정하는데, 현실에서는 잘 지켜지지 않습니다.\n"
            "- **기대수익률 추정은 오차가 큽니다.** 과거에 많이 오른 종목에 비중이 "
            "쏠리는 경향이 있으니, 비중 안정성 점검을 함께 보세요.\n"
            "- **세금은 반영되지 않습니다.** 거래비용만 반영되며, 양도소득세·배당소득세는 "
            "고려하지 않습니다.\n"
            "- **생존편향** — 상장폐지된 종목은 데이터에 없으므로, 과거 성과가 실제보다 "
            "좋아 보일 수 있습니다.\n"
            "- **데이터 출처는 야후 파이낸스**이며, 특히 국내 종목의 배당 조정이 "
            "부정확한 경우가 있습니다.")
